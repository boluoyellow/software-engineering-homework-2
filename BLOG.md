...
| 子任务 | 借助何种 AIGC 技术 | 我提出的要求与 AI 提供的内容 | 实际效果 | 人工修改 |
| --- | --- | --- | --- | --- |
| 弯箭头与消除动画 | Codex | 要求箭头能够多次转弯，并在消除时像抽绳子一样整体移动；AI 编写了路径采样和平滑移动代码 | 基本实现了完整箭身移动，但第一版箭头会突然缩短，转弯也不够自然 | 根据运行画面反复调整路径长度、转弯半径和移动速度 |
| 随机关卡生成 | Codex | 要求地图随机、箭头丰富并保证有解；AI 先后实现了长条铺设、矩形分区和随机路径方案 | 前两版虽然可解，但排列过于整齐，与参考效果差距较大 | 继续提出修改意见，最终允许地图留空，并调整箭头数量和密度 |
| 中文界面与字体问题 | Codex | 要求删除英文说明并将界面改成中文；AI 修改了界面文字和字体加载方式 | 中文界面可以显示，但系统字体扫描在当前电脑上出现报错 | 根据报错信息改为直接加载 Windows 中文字体文件，并重新测试 |
| 地图测试与求解检查 | Codex | 要求验证随机地图不会重叠、不会产生死局；AI 补充了求解器和自动测试代码 | 单张地图测试正常，但不能说明随机生成一直可靠 | 额外批量生成 200 张地图进行压力测试，确认都可以清空 |
